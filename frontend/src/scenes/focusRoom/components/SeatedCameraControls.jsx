...
export function SeatedCameraControls({ enabled = true }) {
  ...
  useEffect(() => {
    const shouldIgnorePointerTarget = (target) => {
      ...
      if (target.closest(".auth-modal-backdrop")) return true;
      if (target.closest(".settings-drawer-backdrop")) return true;
      if (target.closest(".focus-board-popup-backdrop")) return true;
      if (target.closest(".focus-stats-card")) return true;
      if (target.closest(".currently-playing-shell")) return true;
      if (target.closest(".focus-room-hud")) return true;
      if (target.closest(".settings-gear")) return true;
      if (target.closest(".focus-board-screen-shell")) return true;
      if (
        target.closest(
